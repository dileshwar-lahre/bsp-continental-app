...
import { 
  ...
  FiPhone, FiMail, FiSend, FiMessageSquare, FiCheckCircle, FiEye, FiDownload, FiXCircle
} from 'react-icons/fi';
...
export default function AdminApprovedPage() {
  ...
  const fetchApprovedRequests = async () => {
    ...
    try {
      ...
      if (response.ok && result.success) {
        const approvedOnly = (result.data || []).filter(r => r.status === 'Approved');
  ...
  const triggerForceDownload = async (fileUrl, fileName) => {
    try {
      ...
      a.href = blobUrl;
      a.download = fileName || 'Approved_Document.pdf';
  ...
  if (status === 'loading' || loading) {
    return (
      <div className="min-h-screen bg-[#F8FAFC] flex flex-col justify-center items-center gap-3 font-sans">
        ...
        <span className="text-xs font-black text-slate-400 uppercase tracking-widest">Loading Approved Archive...</span>
      </div>
    );
  }
      ...
      <div className="w-full max-w-5xl space-y-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white border border-slate-100 p-6 rounded-3xl shadow-xs">
          <div className="space-y-1">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-lg md:text-xl font-black text-emerald-600 uppercase tracking-tight flex items-center gap-2">
                  <FiCheckCircle size={20} /> APPROVED APPLICATIONS ARCHIVE
                </h1>
                <p className="text-xs font-bold text-slate-400">
                  Total Approved Records: <span className="text-emerald-600 font-black">{approvedRequests.length}</span>
                </p>
              </div>
            </div>
          </div>

          <button onClick={fetchApprovedRequests} disabled={loading} className="inline-flex items-center justify-center gap-2 px-5 py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-black text-xs rounded-2xl uppercase tracking-wider transition-all cursor-pointer shadow-xs">
        ...
        {approvedRequests.length === 0 ? (
          <div className="bg-white border border-slate-100 rounded-3xl p-12 text-center space-y-3 shadow-xs">
            ...
            <h3 className="text-sm font-black text-slate-800 uppercase">No Approved Records Yet</h3>
          </div>
        ) : (
          <div className="space-y-5">
            {approvedRequests.map((req) => (
              <div key={req._id} className="bg-white border border-emerald-100 rounded-3xl p-6 shadow-xs space-y-5 hover:border-emerald-200 transition-all">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3.5 border-b border-slate-100">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="px-3.5 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-wider bg-emerald-50 text-emerald-700 border border-emerald-200">
                      ● Approved & Cleared
                    </span>

                    <button onClick={() => openReplyBox(req)} className="inline-flex items-center gap-1.5 px-3.5 py-1.5 bg-slate-900 hover:bg-emerald-700 text-white font-black text-[10px] rounded-xl uppercase transition-all cursor-pointer">
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-5 text-xs">
                  <div className="md:col-span-4 bg-slate-50/70 p-4 rounded-2xl border border-slate-100 space-y-2">
                    ...
                    <p className="font-black text-slate-900 flex items-center gap-2"><FiUser size={14} className="text-emerald-600" /> {req.userName || 'Anonymous'}</p>
                ...
                {/* Sent Approval Certificate Display */}
                {req.adminReply?.message && (
                  <div className="bg-emerald-50/80 border border-emerald-200 p-4 rounded-2xl space-y-2">
                    <span className="text-[9px] font-black text-emerald-800 uppercase tracking-widest block">SENT APPROVAL CERTIFICATE & MESSAGE</span>
                    <p className="text-xs font-bold text-slate-800">"{req.adminReply.message}"</p>
                    {req.adminReply.document?.url && (
                      <div className="flex items-center gap-2 pt-1">
                        <a href={req.adminReply.document.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 bg-emerald-700 text-white px-3 py-1.5 rounded-xl text-[10px] font-black uppercase">
                          <FiEye size={12} /> View Certificate
                        </a>
                        <button onClick={() => triggerForceDownload(req.adminReply.document.url, req.adminReply.document.name || 'Verified_Approval_Certificate.pdf')} className="inline-flex items-center gap-1.5 bg-slate-900 text-white px-3 py-1.5 rounded-xl text-[10px] font-black uppercase cursor-pointer">
                ...
                {activeReplyId === req._id && (
                  <div className="bg-slate-950 text-white p-6 rounded-3xl space-y-4 border border-slate-800">
                    <div className="flex items-center justify-between pb-2 border-b border-slate-800">
                      <h4 className="text-xs font-black uppercase text-indigo-400">CHANGE DECISION FROM APPROVED</h4>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                        ...
                        <select value={replyStatus} onChange={(e) => setReplyStatus(e.target.value)} className="w-full bg-slate-900 border border-slate-700 rounded-xl px-3 py-2.5 text-xs font-bold text-white outline-none">
                          <option value="Approved">Keep Approved</option>
                          <option value="In Review">Move to In Review Desk</option>
                          <option value="Rejected">Reject & Move to Rejected Archive</option>
                        </select>
                      </div>

                      <div className="space-y-1.5">
                        <label className="text-[10px] font-black uppercase text-slate-400">Update Certificate / PDF</label>
